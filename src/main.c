...
int main(void)
{
	...
	bitmap_error_t error = bitmapReadPixels("img/test.bmp", (bitmap_pixel_t**)&pixels, &widthPx, &heightPx, BITMAP_COLOR_SPACE_RGB);
	...
	error = bitmapWritePixels("img/test.mod.bmp", BITMAP_BOOL_TRUE, &params, (bitmap_pixel_t*)pixels);
	assert(error == BITMAP_ERROR_SUCCESS);
